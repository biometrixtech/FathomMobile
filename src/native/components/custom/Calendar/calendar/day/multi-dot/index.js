/*
 * @Author: Vir Desai 
 * @Date: 2018-04-23 15:05:16 
 * @Last Modified by:   Vir Desai 
 * @Last Modified time: 2018-04-23 15:05:16 
 */
  ...
  renderDots(marking) {
      ...
      if (marking.dots && Array.isArray(marking.dots) && marking.dots.length > 0) {
          ...
          return validDots.map((dot, index) => {
              return (
                  <View key={dot.key ? dot.key : index} style={[baseDotStyle, { backgroundColor: marking.selected && dot.selectedDotColor ? dot.selectedDotColor : dot.color}]}/>
              );
          });
      }
      return null;
