    ...
    constructor(props) {
        ...
        let currentIndex = props.dataType === 0 ?
            ...
            : props.dataType === 1 ?
                ...
                :
                _.findLastIndex(this.props.plan.dailyPlan[0].trends.biomechanics_hip_drop.sessions);
        this.state  = {
    ...
    _toggleRichDataView = () => {
        ...
        let sessions = dataType === 0 ?
            ...
            : dataType === 1 ?
                ...
                :
                plan.dailyPlan[0].trends.biomechanics_hip_drop.sessions;
        let asymmetryIndex = dataType === 0 ? 'apt' : dataType === 1 ? 'ankle_pitch' : 'hip_drop';
    ...
    render = () => {
        ...
        if(dataType === 0) {
                ...
                <View style={{backgroundColor: AppColors.white, flex: 1,}}>
                    ...
                    >
                        ...
                        <View style={{marginHorizontal: AppSizes.paddingMed, marginTop: AppSizes.paddingMed,}}>
                            <Text robotoLight style={{color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(14), marginBottom: AppSizes.paddingSml, marginHorizontal: (AppSizes.paddingLrg - AppSizes.paddingMed),}}>{'Average Range of Motion (ROM)'}</Text>
                            <BiomechanicsCharts
                                dataType={dataType}
                                pieDetails={{
                                    leftPieInnerRadius,
            ...
            let {
                ...
                sessions,
                updatedChartData,
            } = PlanLogic.handleBiomechanicsHipDropRenderLogic(plan, currentIndex);
            return (
                <View style={{backgroundColor: AppColors.white, flex: 1,}}>
                    ...
                    >
                        ...
                        <View style={{alignItems: 'center', flexDirection: 'row', justifyContent: 'center', marginHorizontal: AppSizes.paddingMed,}}>
                            <TabIcon
                                ...
                                type={'material-community'}
                            />
                            <Text robotoRegular style={{color: AppColors.zeplin.slate, flex: 8, fontSize: AppFonts.scaleFont(24), textAlign: 'center',}}>{'Hip Drop'}</Text>
                            <TabIcon
                                color={AppColors.zeplin.slateXLight}
                                containerStyle={[{flex: 1,}]}
                                icon={'help-circle-outline'}
                        ...
                        <ParsedText
                            ...
                            style={[AppStyles.robotoLight, {color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(18), marginHorizontal: AppSizes.paddingLrg,},]}
                        >
                            {selectedHipDropSession.asymmetry.hip_drop.detail_text ? selectedHipDropSession.asymmetry.hip_drop.detail_text : ''}
                        </ParsedText>
                    ...
                    >
                        <View style={{flex: 1, flexDirection: 'column',}}>
                            ...
                            <View style={{backgroundColor: AppColors.white,}}>
                                <View style={{backgroundColor: AppColors.zeplin.superLight, flexDirection: 'row', padding: AppSizes.padding,}}>
                                    <Text robotoMedium style={{color: AppColors.zeplin.slate, flex: 9, fontSize: AppFonts.scaleFont(22),}}>{'Hip Drop Asymmetry'}</Text>
                                    <TabIcon
                                        containerStyle={[{flex: 1,}]}
                                        icon={'close'}
                                        iconStyle={[{color: `${AppColors.black}${PlanLogic.returnHexOpacity(0.3)}`,}]}
                                ...
                                <ScrollView style={{padding: AppSizes.paddingLrg,}}>
                                    <Text robotoBold style={{color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(14),}}>{'What is Hip Drop Asymmetry?'}</Text>
                                    <Text robotoLight style={{color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(14),}}>{'Hip Drop asymmetry is often caused by a combination of imbalances between muscles that are tight or overactive (like the psoas, adductor, and lateral hamstring) and other muscles that are comparatively weak or inhibited (like the glute med, hip external rotators, and local stabilizers).\n\nThis may be part of a lower-extremity movement dysfunction affecting the Lumbopelvic Hip Complex which leads to predictable patterns of injury such as plantar fasciitis, shin splints, anterior knee pain, and lower back pain.\n\nCombined with your other movement and response data, we try to identify your most likely imbalances and the best corrective exercises to efficiently correct them.'}</Text>
                                    <Spacer size={AppSizes.padding} />
                                    <Text robotoBold style={{color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(14),}}>{'How It\'s Measured:'}</Text>
                                    <Text robotoLight style={{color: AppColors.zeplin.slate, fontSize: AppFonts.scaleFont(14),}}>{'Hip Drop measures the angular range of motion of your hip complex as is rotates in the frontal plane (up and down) while each foot is in contact with the ground. We compare this range of motion for left foot vs right foot ground contacts to identify asymmetry and try to identify the source of the imbalances.\n\nFor best results, remember to consistently place your "hip sensor" in the center of your spine and low on your sacrum, just above the tailbone.'}</Text>
                                </ScrollView>
                            </View>
                        </View>
                    </SlidingUpPanel>
